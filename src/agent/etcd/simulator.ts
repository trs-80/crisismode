...
export class EtcdSimulator implements EtcdBackend {
  ...
  async evaluateCheck(check: CheckExpression): Promise<boolean> {
    ...
    // Fail closed (matching the llm-provider/vector-store precedent): a
    // precondition/success-criteria check on an unrecognized statement is a
    // plan-authoring bug, and this backend must not let it pass silently.
    // Throwing was considered instead, but the graph engine's node functions
    // (src/framework/graph-nodes.ts) call evaluateCheck without a
    // surrounding try/catch — an exception here would propagate out of
    // LangGraph's stream() uncaught rather than surface as a failed step, so
    // `false` is the only semantic both execution engines handle safely.
    return false;
  }
